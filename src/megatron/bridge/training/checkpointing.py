...
def _reset_logging_locks_in_child() -> None:
    """Reset ``logging`` locks a peer thread may have held at ``fork`` time.

    Without this the child deadlocks on its first log call.
    """
...
def schedule_forked_save(global_state: GlobalState, async_request: AsyncRequest) -> None:
    ...
    Parent blocks in ``waitpid`` (GIL-releasing) so a co-resident event loop
    keeps running; child inherits CPU state via COW and must not touch
    CUDA/NCCL. Parent runs ``dist.barrier`` + ``finalize_fns`` after the
    child exits — those need the parent's NCCL state.
    """
    del global_state
    # Preload touches CUDA — must be parent-side.
