...
class Glm5NextTransformerLayer(TransformerLayer):
    """Transformer layer with manifold-constrained hyper-connections (mHC), MoE-safe.

    GLM-5.3 replaces the ordinary residual stream with mHC: the hidden state carries
    ``mhc_num_residual_streams`` parallel streams, each sublayer reads a learned
    aggregation of them and writes back a learned expansion, and the streams are mixed
    by a doubly-stochastic matrix produced by Sinkhorn-Knopp normalization.

    Why this class exists rather than ``HyperConnectionTransformerLayer``
    --------------------------------------------------------------------
    Megatron-Core ships that layer, and its mHC math is exactly what GLM-5.3 needs, but
    its ``__init__`` refuses any MoE layer::

        if self.is_moe_layer:
            raise NotImplementedError(
                "HyperConnectionTransformerLayer does not support MoE MLP submodules. ...")

    GLM-5.3 is 42 MoE layers out of 45, so that covers almost the whole model. The
    restriction arrived with the original mHC commit ("[Split 1/N of #3430] feat(mHC):
    basic pytorch implementation of manifold hyper connection") and is still present
    upstream; the error points at a ``HyperConnectionHybridLayer`` that exists in neither
    this pin nor upstream.

    The restriction is on the *fused* residual path, not on the mHC operators.
    ``HyperConnectionModule`` exposes an unfused API -- ``compute_mappings``,
    ``aggregate``, ``apply_h_res``, ``apply_h_post`` -- which this layer composes
    explicitly. ``hyper_connection.py`` in this pin is byte-identical to upstream main,
    so nothing here depends on a fork-only patch and the layer works against either.

    How it hooks in
    ---------------
    ``TransformerLayer`` already anticipates an mHC subclass: ``_run_input_layernorm``
    documents its third return value as "an opaque payload subclasses can use to thread
    extra intermediates (e.g. mHC ``h_res``/``h_post``)". This overrides the four hooks
    that payload flows through and leaves the layer skeleton alone.

    Per sublayer the sequence is::

        residual                  = hidden_states            # [s, b, n*C]
        aggregated, h_res, h_post = hyper_connection(hidden_states)
        out_with_bias             = sublayer(norm(aggregated))
        hidden_states             = bda(apply_h_post(out_with_bias, h_post),
                                        apply_h_res(h_res, residual))

    which is what ``fused_h_res_h_post_bda`` fuses, and matches the reference term for
    term (ref: modular:1142 ``Glm5NextTextDecoderLayer``, whose update is
    ``post * out + comb^T @ residual`` with ``post``/``comb`` being ``h_post``/``h_res``).

    Not carried over from the fused path: mHC activation recompute
    (``mhc_recompute_layer_num``) and layernorm recompute. Both are memory
    optimizations rather than correctness, and both deserve measuring before they are
    worth the extra state machine.
    """
    ...
    def __init__(
        ...
        if not config.enable_mhc_connections:
            raise ValueError(
                "Glm5NextTransformerLayer implements mHC residual streams and requires "
                "enable_mhc_connections=True. Use TransformerLayer if mHC is not wanted."
            )

        # Two sites per layer, matching the reference: one around attention, one around
        # the MLP. Built here rather than read from submodules because this layer always
        # needs both -- there is no configuration of GLM-5.3 with only one.
        self.self_attention_hyper_connection = HyperConnectionModule(config=config, layer_number=layer_number)
        self.mlp_hyper_connection = HyperConnectionModule(config=config, layer_number=layer_number)

    # ------------------------------------------------------------------ attention

    def _run_input_layernorm(self, hidden_states):
        """Aggregate the streams, then normalize the single aggregated stream.

        Returns ``(input_layernorm_output, residual, (h_res, h_post))``. ``residual`` is
        the *n-stream* tensor captured before aggregation -- the mixing in
        ``_apply_self_attn_bda_step`` needs all streams, not the aggregate.
        """
        residual = hidden_states

        aggregated, h_res, h_post = self.self_attention_hyper_connection(hidden_states)

        # Layernorm recompute is not wired on the mHC path; see the class docstring. The
        # flag is still set because the base skeleton reads it after this hook returns.
        self._input_layernorm_checkpoint_active = False
        self.attn_norm_manager = self.off_interface(self.offload_attn_norm, aggregated, "attn_norm")
        with self.attn_norm_manager as aggregated:
            input_layernorm_output = self.input_layernorm(aggregated)

        if isinstance(input_layernorm_output, tuple):
            raise ValueError(
                "Glm5NextTransformerLayer does not support a residual-returning "
                "input_layernorm: the residual it returns is the aggregated single "
                "stream, but the mHC update mixes the n-stream tensor."
            )
    ...
    def _apply_self_attn_bda_step(self, attention_output_with_bias, residual, attn_state=()):
        """Unfused ``apply_h_res`` + ``apply_h_post`` + bias-dropout-add."""
        if not attn_state:
            raise ValueError(
                "Glm5NextTransformerLayer._apply_self_attn_bda_step requires the "
                "(h_res, h_post) payload produced by _run_input_layernorm; it was reached "
                "through a base-class path that does not thread mHC intermediates."
            )
        h_res, h_post = attn_state
        hidden_states = self._mhc_commit(
    ...
    def _pre_mlp_layernorm_and_residual(self, hidden_states):
        """Same aggregation for the MLP site, threading its own ``(h_res, h_post)``."""
        residual = hidden_states

        aggregated, mlp_h_res, mlp_h_post = self.mlp_hyper_connection(hidden_states)

        self.mlp_norm_manager = self.off_interface(self.offload_mlp_norm, aggregated, "mlp_norm")
        with self.mlp_norm_manager as aggregated:
            pre_mlp_layernorm_output = self.pre_mlp_layernorm(aggregated)

        if isinstance(pre_mlp_layernorm_output, tuple):
            raise ValueError(
                "Glm5NextTransformerLayer does not support a residual-returning "
                "pre_mlp_layernorm; see _run_input_layernorm for why."
            )

        if self.config.fp32_residual_connection:
            residual = residual.float()
        return pre_mlp_layernorm_output, residual, (mlp_h_res, mlp_h_post)

    def _apply_mlp_bda_step(self, mlp_output_with_bias, residual, mlp_state=()):
        """Unfused ``apply_h_res`` + ``apply_h_post`` + bias-dropout-add, MLP site."""
        if not mlp_state:
            raise ValueError(
                "Glm5NextTransformerLayer._apply_mlp_bda_step requires the "
                "(h_res, h_post) payload produced by _pre_mlp_layernorm_and_residual; it "
                "was reached through a base-class path that does not thread mHC "
                "intermediates."
            )
        mlp_h_res, mlp_h_post = mlp_state
        hidden_states = self._mhc_commit(
    ...
    def _mhc_commit(self, hyper_connection, bda_spec, output_with_bias, residual, h_res, h_post):
        """Expand the sublayer output back to n streams and mix it with the residual.

        ``apply_h_post`` widens ``(output, bias)`` from ``[s, b, C]`` to ``[s, b, n*C]``,
        carrying the bias so the bias-dropout-add still sees a matching pair.
        ``apply_h_res`` mixes the incoming n-stream residual through the doubly-stochastic
        matrix. The ordinary bda then adds them, exactly as it would a plain residual.
        """
        with self.bias_dropout_add_exec_handler():
            expanded_with_bias = hyper_connection.apply_h_post(output_with_bias, h_post)
            mixed_residual = hyper_connection.apply_h_res(h_res, residual)
            return bda_spec(self.training, self.config.bias_dropout_fusion)(
                expanded_with_bias, mixed_residual, self.hidden_dropout
            )
