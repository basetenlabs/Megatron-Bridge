...
class MegatronModelBridge(
    ...
    def maybe_load_native_hf_weight(
        ...
        """Optionally load a checkpoint tensor directly into native parameter storage.

        Subclasses can override this hook for storage formats that must bypass
        the standard tensor transformation and ``copy_`` path.

        Args:
            task: Resolved HF-to-Megatron conversion task.
            hf_state_dict: Lazy HuggingFace checkpoint tensor mapping.
